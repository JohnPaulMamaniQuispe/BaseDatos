...
insert into actors(id, name, gender) values(1,'John Mamani','M');
insert into actors(id, name, gender) values(2,'Andres Flores','M');
insert into actors(id, name, gender) values(3,'Nilda Sharol','F');
insert into actors(id, name, gender) values(4,'Angie Ruiz ','F');

insert into movie_cast(actor_id, movie_id, role) values(1,1,'Actor');
insert into movie_cast(actor_id, movie_id, role) values(2,2,'Actor');
insert into movie_cast(actor_id, movie_id, role) values(3,3,'Actor');
insert into movie_cast(actor_id, movie_id, role) values(4,4,'Actor');
...
insert into movies( id,year,title, language, duration, director_id, genre_id) values(10,2016,'Silence', 'English',120, 3,1)
...
---1Crear un procedimiento almacenado para que pueda ingresar películas. 
---Se debe validar que no se ingrese películas repetidas. En el caso que se repita, 
---se debe mostrar un mensaje de alerta de película repetida.
...
exec INSERTAR_MOVIES 12 ,Abuela,2016,English,120, 3,1
...
select genre_id, count(1) as cantidad_genero_por
from movies group by genre_id --


CREATE FUNCTION can(@id_actor int ,@id_genero int) RETURNS REAL
AS
BEGIN
DECLARE @cantidad_actors int
DECLARE @GENEROS int
DECLARE @TOTAL int
select @cantidad_actors = count (director_id) from movies where director_id = @id_actor
select @GENEROS= count (genre_id) from movies where genre_id =@id_genero
select @TOTAL =  @cantidad_actors  + @GENEROS 
RETURN @TOTAL
END

SELECT id, title , language, dbo.can(1,10) as antidad_actores FROM movies 
...
begin 
	declare @quantity int
	select  @quantity = count(a.id) from actors a
...
select dbo.count_actor_por_genreanio('Terror',2000) as 'Cantidad Actores' from actors
...
as return
	...
	where quantity = (select max(quantity) from dbo.f_comments_per_genre(@description)));
select * from dbo.f_reviewer_max_comments_per_genre('drama');
--nombre actor con mas participaciÛn en peliculas de un genero, aÒo parametros ambos
...
select * from dbo.f_max_actor_per_genre_anio('Ciencia ficciÛn', 2000)
